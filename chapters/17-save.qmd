...
Note the ordering: `newHigh` is *latched* before the overwrite,
because after it `score > Attract.high` is false and a screen that
re-derived the answer at draw time would never show the banner.
Compute a fact at the moment it becomes true; persist it and display
it, but do not ask a mutated store to remember it for you.
...
- @sec-lore turns that JSON key caveat into an architecture: when the
  save file *is* the game's memory, you build string keys at the write,
  not repair integer ones at the load.
