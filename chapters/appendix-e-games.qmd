...
Playdate games — more than sixty of them, built by the author on the
...
The catalog is in two halves: sixteen standalone games, then the seven
*collections* — shared engines with a fleet of games on top, forty-nine
games between them — plus one reusable module. Five of those
collections get a whole chapter to themselves in Part VII. Screenshots
are the games' own repository `screenshot.png` files, upscaled 2x
nearest-neighbor like every other figure in the book.
...
trusts it for the timer coverage the other sixty-three games skip.
...
A sheepdog trial: steer the dog with the crank, creep and bark to
drive a mixed flock of sheep, goats, ducks and geese past rocks,
walls and ponds through a funnel race into the pen before the clock
runs out — four dog personas, eight boids-driven breeds, twelve
trials.

![Flock: the dog works a scattering flock down the funnel toward the
pen, the camera zoomed out to hold them all.](../figures/case-studies/flock.png){#fig-cs-flock}

**Cited in:** @sec-camera, as the book's one *zoom-to-fit* camera —
the fighting-game / *Samurai Shodown* pattern that fits a bounding box
of the whole cast (dog plus every unpenned animal) instead of chasing
a single target. It is the chapter's witness for why a zooming camera
on the Playdate cannot ride the `setDrawOffset` sandwich (no hardware
scale), must bake the zoom into a hand-rolled world→screen transform,
and therefore draws its entire cast as parametric 1-bit geometry that
stays crisp at any scale.

### The supporting bestiary

Seven more standalones inform chapters without being quoted line by
...
Seven repositories are not single games but engines with fleets on
top. When a chapter quotes `vec/` or `tiles/core/`, it is quoting
code that a dozen games run through every frame — the strongest
battle-testing in the catalog. Six of the seven — Phosphor, Tiles,
Voxel, Dither, Lore, and Shmup — are read end to end in Part VII, each
in a chapter of its own with the engine vendored into a runnable
example.
...
soccer game, a brawler, and five more. Less quoted than lived-in: this
port marathon (ten games in one push) is where the headless
...
The RPG engine: story state over a chunked tile world — a big walkable
overworld, people to talk to, a party that grows, and battles in **two
grammars** (menu-driven turns in a scene, real-time swings in the
field) over one shared character sheet. Three games: *Slice* (the
vertical slice and living engine demo — a chunked overworld, NPCs, a
shop, a chest, a battle, a save), *Shrew* (Dragon Quest grammar: a
shrew knight must eat half her body weight by dawn — overworld, town,
boat, two-floor burrow, the Mole Tyrant), and *Stoat* (Secret of Mana
grammar: charge swings, pounces, telegraphed rats, and a Rat King who
stops mid-fight to talk).

![Shrew, from the Lore collection: the dawn meter, the chunked
overworld, and twelve grams to
eat.](../figures/case-studies/shrew.png){#fig-cs-shrew}

**Cited in:** @sec-lore — an entire chapter walks the `core/` engine
module by module (the chunked LRU map cache, the state stack that makes
menus-over-dialogs-over-shops free, the coroutine screenplay runner
whose primitives also write the smoke autopilot, the one character
sheet under both battle engines, the pattern-and-order-list music with
its resume-mid-phrase stinger, and all three games) and is this
collection's primary citation. It builds on @sec-tiles (`lmap` descends
from `tmap`: same char grids, same write barrier, chunked), @sec-camera
(`lcam` is `tcam` plus a scripted pan), @sec-save (its `lstate` ledger
is the string-key JSON lesson made structural), and @sec-harness, whose
autopilot argument it takes to the limit — including the war story of
the bot that farmed a boss's summons for 57 kills without ever touching
the boss, and `tools/headless.lua`, a stubbed-SDK Lua runner that beats
a whole RPG in two seconds.

### Shmup

A 1-bit shoot-'em-up engine built around **the frame of reference** —
what is nailed down and what moves — with one game per frame: *Nova
Strike* (`vertical`: the world falls past a screen-locked ship, in the
line of *Xevious*), *Ravine* (`side`: a forced-scroll cave-flyer with
Scramble-style terrain, draining fuel and ballistic bombs), and
*Skimmer* (`free`: the level is a fixed-width place with two ends and
the camera chases you, in the line of *Uridium* — a different *Skimmer*
from Dither's dragonfly; the fleet reuses a good name). Games on it are
almost entirely *data*: an enemy type composes one movement behavior
with one fire behavior, a level is a spawn timeline, and the whole
paradigm is a single string in a table.

**Cited in:** @sec-shmup, which reads the engine end to end and vendors
it into that chapter's example. Its house rule is worth stating on its
own: solid white sprites on black space, code-drawn, because that is
the lesson a 1-bit OpenTyrian port teaches — at bullet size the eye
cannot resolve a dither pattern, so it resolves the average, and the
average of a 50% dither is exactly the grey smudge you most need your
bullets not to be (@sec-dither).
