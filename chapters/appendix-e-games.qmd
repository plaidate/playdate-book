...
The catalog is in two halves: the standalone games, then the four
...
Four repositories are not single games but engines with fleets on
...
**Cited in:** @sec-images (the `beams.lua` stroke font — text drawn
as line segments, no bitmaps), @sec-wireframe (its `mat.lua` rotation
matrices, `proj.lua` near-plane clipping, and `shapes.lua` asteroid
...
**Cited in:** @sec-wireframe (the two-add 3/4 projection, and the
collection's design rule quoted there: *a voxel game must use height
and volume — dig, lob, climb, flood, fall — to earn the engine*),
@sec-crank (*Lob*'s wind-and-release crank mapping), and @sec-maze
(*Lob* again: area-of-effect tools must check for friendlies,
thrower included).
...
**Cited in:** more chapters than anything except molt — @sec-images
(the fully procedural tileset in `tmap.lua`: four games shipped
without a drawing tool ever being opened), @sec-camera (`tcam.lua` lerp-and-deadzone
camera and the pre-rendered background repaint strategy), @sec-sfx
(`tsnd.lua` noise sweeps), @sec-platformer (`tphys.lua` — the fixed-DT
tile physics and 1px substep at that chapter's core, plus *Blast*'s
walkable-bomb `isSolid` and *Burrow*'s falling-boulder variant), and
@sec-maze (*Blast*'s radius checks). *Relic*'s save-versioning
appears in @sec-save.
